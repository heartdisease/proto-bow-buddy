  ...
  storage: (function() {
        ...
        return close().then(() => {
          return new Promise((resolve, reject) => {
            (function tryDeleteDb() {
              ...
              const deletionRequest = window.indexedDB.deleteDatabase(
                "BowBuddyDb"
              );
    ...
    return {
      getPlayers: function() {
        return db()
          .transaction("players")
          .then(playerObjectStore => fetchAll(playerObjectStore));
      },

      getPlayer: function(pid) {
        return db()
          .transaction("players")
          .then(playerObjectStore => fetchById(playerObjectStore, "pid", pid));
      },
      ...
      getPlayersWithScore: function(gid, station) {
        return db()
          ...
          .then(objectStores => {
            return fetchById(objectStores.games, "gid", gid).then(game =>
              fetchAll(
                ...
                .then(scoreObjectStore => {
                  ...
                  return new Promise((resolve, reject) => {
                    players.forEach(player => {
                      fetchById(scoreObjectStore, "sid", [
                        gid,
                        player.pid,
                        station
                      ]).then(score => {
                        playersWithScore.push(
                          score && score.score
                            ? Object.assign(player, { score: score.score })
                            : player
                        );
      ...
      addPlayer: function(name, email) {
        return db()
          ...
          .then(playerObjectStore => {
            ...
            return new Promise((resolve, reject) => {
              request.onsuccess = event =>
                resolve({ pid: event.target.result, name: name, email: email });
      ...
      getCourses: function() {
        return db()
          .transaction("courses")
          .then(courseObjectStore => fetchAll(courseObjectStore));
      },
      ...
      getCourseForGame: function(gid) {
        return db()
          ...
          .then(objectStores =>
            fetchById(objectStores.games, "gid", gid).then(game =>
              fetchById(objectStores.courses, "cid", game.cid)
            )
          );
      },
      ...
      getGames: function() {
        return db()
          .transaction("games")
          .then(gameObjectStore => fetchAll(gameObjectStore));
      },

      getGame: function(gid) {
        return db()
          .transaction("games")
          .then(gameObjectStore => fetchById(gameObjectStore, "gid", gid));
      },
          ...
          return dbRef.transaction(objectStoreNames).then(objectStores => {
            ...
            return new Promise((resolve, reject) => {
              objectStoreNames.forEach(objectStoreName => {
                return fetchAll(objectStores[objectStoreName]).then(records => {
      ...
      importDb: function(dbObject) {
        ...
        return db()
          ...
          .then(e => {
            ...
            console.log(
              ">> Step 2: Requested transactions for " + objectStoreNames
            );
            ...
            return db()
              ...
              .then(objectStores => {
                ...
                console.log(
                  ">> Step 2.1: We now have all the requested object stores"
                );
                ...
                return new Promise((resolve, reject) => {
                  objectStoreNames.forEach(objectStoreName => {
                    console.log(
                      ">> Step " +
                        ++steps +
                        ": Add all data records into object storage '" +
                        objectStoreName +
                        "'"
                    );
                    ...
                    dataRecords.forEach(dataRecord => {
                      let addRequest = objectStores[objectStoreName].add(
                        dataRecord
                      );
                      ...
                      addRequest.onsuccess = e => {
                        ...
                        console.log(
                          "objectStoresCompleted: " + objectStoresCompleted
                        );
